# Sprint 3.5: UI/UX Refinement — Implementation Details (Archive)

**Status:** ✅ **COMPLETE** | **Dates:** Dec 6-18, 2025
**Goal:** Polish HERMES Messaging + Forum UI/UX before Sprint 4

> 📄 **Summary:** [03.5-sprint-3.5-ui-refinement.md](../../03.5-sprint-3.5-ui-refinement.md)
> 🎨 **Color Panel:** [dev-color-panel.md](../../dev-color-panel.md)
...
- [x] **LOC Cap:** 350 lines/file ✅ (max 346 lines — CommentStream.tsx)
- [x] **Coverage:** 100% Line/Branch/Function/Statement ✅ (335 tests)
- [x] **Browser-Safe:** No `node:*` imports, CSS-only animations ✅
- [x] **E2E Mode:** Works with `VITE_E2E_MODE=true` ✅
...
### 2.2 VENN → Forum Transition (3D Card Flip)

**Vision:** Article summary flips to reveal debate discussion. Headline stays anchored.

<details>
<summary>📐 Layout Diagrams (click to expand)</summary>
...
├──────────────────────────────────────────────────────────────┤
│ ┌──────────────────────────────────────────────────────────┐ │
│ │   ARTICLE SUMMARY + BIAS TABLE                           │ │
│ │   [Frame: "Historic victory"] [Reframe: "Insufficient"]  │ │ ← FLIPS
│ │   [💬 Discuss in Forum]                                  │ │
│ └──────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────┘
                    ↓ 3D Card Flip ↓
...
┌──────────────────────────────────────────────────────────────┐
│ Headline: "Climate Bill Passes Senate"                       │ ← SAME
├──────────────────────────────────────────────────────────────┤
│ ┌──────────────────────────────────────────────────────────┐ │
...
│░░┌────────────────────────────────────────────────────────┐░░│
│░░│  @alice • "Finally, logic prevails!"         [▲12 ▼]  │░░│
│░░│  ┌──────────────────┬────────────────────────────────┐│░░│
│░░│  │ 👍 CONCUR (8)    │ 👎 COUNTER (4)                ││░░│
...
4. **Action Buttons:** "Send to Rep", "Draft Proposal", "Start Project" → modal "Coming in Sprint 4"
...
**Behavior:** Click comment → CSS scale/z-index animation → fills width → shows nested Concur/Counter
- **Escape:** Zoom out one level
- **Backdrop click:** Return to root
- **Parent edge click:** Return to that level
- **Pills:** Collapsed comments show "👍 3 Concur" pills; expanded shows full columns
...
- **Navbar icon:** 💬 accessible from any page → `/hermes/messages`
...
- [x] CSS-only animations (transform, opacity) → 60fps ✅
- [x] `prefers-reduced-motion` respected ✅
- [x] `aria-expanded`, `aria-hidden`, `role="dialog"` on flip/zoom ✅
- [x] Focus trap in modals, Escape key dismissal ✅
- [ ] Lighthouse A11y ≥ 90
...
|-------|-------------|--------|
| 1 | Schema v1 (stance), Store selectors, Debate UI, Zoom hook | ✅ |
| 2a | User Handles, IDChip | ✅ |
| 2b | Navbar 💬, Community Reaction Summary | ✅ |
| 3 | FlippableCard 3D animation | ✅ |
| 4 | Manual testing bug fixes | ✅ |
| 5 | CI & E2E fixes | ✅ |
| 6 | Design Unification (borderless, themes, 3D flip) | ✅ |
| 6.1 | Visual Refinements (Heroicons, glow system, nested summary) | ✅ |
| 6.2 | Dev Color Panel (43 CSS vars, sliders, save to defaults) | ✅ |
| 6.3 | ThreadView Consolidation (unified card, auto-summary) | ✅ |
| 7.0 | Schema Update (tri-state stance: concur/counter/discuss) | ✅ |
| 7.1 | CommentStream (threaded tree, SlideToPost, stance connectors) | ✅ |

**Test Status:** 335 unit + 10 E2E passing | 100% coverage | CI green ✅

<details>
<summary>📋 Phase Implementation Details (click to expand)</summary>
...
- `utils/handle.ts` — `isValidHandle()`, `getHandleError()`
- `useIdentity.ts` — handle field + `updateHandle()`
- `IDChip.tsx` (71 LOC) — replaces ContactQR
- `HandleEditor.tsx` (55 LOC) — extracted for LOC compliance

### Phase 2b: Navbar & Summary (Dec 7)
- 💬 in header → `/hermes/messages`
...
### Forum
- [x] Two columns (Concur/Counter) display ✅
- [x] Community Summary shows metrics ✅
- [x] Zoom fills width, shows nested debate ✅
- [x] Stacked cards breadcrumb ✅
- [x] 3D flip VENN → Forum ✅
- [x] Add comments from zoomed view ✅
- [ ] Vote persistence
...
| Action buttons → modal | Accessibility > disabled buttons |
| 45s AI debounce | Prevent thrashing local WebLLM |

<details>
<summary>📝 Session Notes Archive (click to expand)</summary>

### Session 1 (Dec 6)
- Issue 2.1: Comment/Counterpoint duplication → debate-first layout
- Issue 2.2: Need VENN → Forum transition → 3D flip
- Issue 2.3: Nested zoom pattern → CSS animation + backdrop
- Issue 3.1: Messaging layout → contacts left, ID Chip fixed
- Issue 3.2: Need handles → alphanumeric 3-20 chars
...
**Round 2:** GO — all concerns addressed
...
5. Navigation too cluttered → simplified routes
...
- **VENN (Front):** News analysis, sentiment capture — soft pastel green lean
- **HERMES (Back):** Community voice, discussion — warm rusty lean
...
```
VENN SIDE (Front) — Pastel Green Lean
┌─────────────────────────────────────────────────────────────┐
│                                          👁️ 12.3   💡 1.6  │ ← metrics (small, top-right)
│                                                             │
│  Article Headline                                           │
│  Summary text...                                            │
│                                                             │
...
│  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  │
│                                                             │
│  [💬 Discuss in Forum]  ← borderless button, shadow only    │
│                                                             │
└─────────────────────────────────────────────────────────────┘

                    ↻ 3D Flip Animation ↻

HERMES SIDE (Back) — Warm Rusty Lean
┌─────────────────────────────────────────────────────────────┐
│                                          👁️ 12.3   💡 1.6  │ ← same metrics position
│                                                             │
│  Thread Title                                               │
│  Post content (lede if from article, or user-written)       │
│                                                             │
│  ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  │ ← hue shift
│                                                             │
│  Conversation Summary                                       │
│  "AI-generated synthesis of the discussion..."              │
│                                                             │
│  [📤] [📝] [🌱]  ← line icons, greyed until threshold       │
│                                                             │
│  ┌─────────────────────┐  ┌─────────────────────┐          │
│  │ Concur (cool)       │  │ Counter (warm)      │          │ ← nested cards
│  │ teal shift          │  │ amber shift         │          │    shadow separation
│  │ shadow only         │  │ shadow only         │          │    NO borders
│  │                     │  │                     │          │
│  │ [+ Add Concur]      │  │ [+ Add Counter]     │          │
│  └─────────────────────┘  └─────────────────────┘          │
│                                                             │
│  [← Back to Analysis]  ← footer position                    │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
| Current | New |
|---------|-----|
| "1 participant • 2 Concur, 1 Counter" text | **Removed** (data kept in store for reports) |
| Bordered cards/columns | **Borderless** — hue shifts + shadows |
| Solid colored buttons | **Borderless buttons** — subtle bg shift + shadow |
| Action buttons open modal | **Line icons** (📤📝🌱) greyed until thresholds |
...
- Mobile layout (vertical stack + horizontal scroll — deferred)
- Specific icon SVGs (will use Heroicons outline set)

### 10.10 Review Feedback (Dec 8, 2025)

**Reviewers:** Codex + Gemini | **Verdict:** ✅ GO
...
| CSS Grid stacking for flip | Gemini | Use `display: grid; grid-area: 1/1` — no JS height calc |
| Hue shifts primary, shadows secondary | Both | Adjust approach — bg delta first, shadow decorative |
...
| Regional = local user or global only | Gemini | Already compliant per §1.2 |
...
#### 1. Conversation Summary — Nested Card
...
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Conversation Summary                                  │  │ ← lighter nested card
│  │ "AI summary text..."                    (font-light)  │  │ ← lighter font weight
│  │                                                       │  │
│  │ [📤] [📝] [🧪]  ← icons only, tooltip on hover        │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  [Concur] [Counter] columns...                              │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 2. Engagement Icons — Glow System
...
**Removed:** "✅ You participated (stance)" text — replaced by icon glow state
...
┌───────────────────────────────────────┐
│ @alice: "Finally, logic!"             │
│ 👍 3 Concur  👎 1 Counter             │ ← pills only
└───────────────────────────────────────┘

Expanded (zoomed):
┌───────────────────────────────────────────────────────────┐
│                                        👁️ 8  💡 0.4      │ ← THIS comment's engagement
│                                                           │
│ @alice: "Finally, logic prevails!"                        │
│ Full content...                                           │
│                                                           │
│ ┌─────────────────┐  ┌─────────────────┐                  │
│ │ Concur (3)      │  │ Counter (1)     │                  │
...
**Note:** Changed from SparklesIcon → **BeakerIcon** for "Start Project"
...
|------|-----------|-------------|--------|
| 1 | `useViewTracking` | Hook: 5s timer + scroll detection → calls `recordRead` | ✅ |
| 2 | `EngagementIcons` | Component: outline/solid + glow (drop-shadow) | ✅ |
| 3 | `ThreadView` | Use `useViewTracking`, icons top-right | ✅ |
| 4 | `CommentCard` | Show engagement icons when `isExpanded` | ✅ |
| 5 | `CommunityReactionSummary` | Nested card, lighter font, icons with `aria-label` | ✅ |
| 6 | Action icons | BeakerIcon, `aria-label` + `title`, keyboard handlers | ✅ |
| 7 | Surface tokens | Light mode variants via CSS variables | ✅ |
| 8 | Icon shadow | Drop shadow behind glow for edge definition | ✅ |
...
|-------|----------------|------------|
| **Integration** | Use existing `recordRead` in `useSentimentState` | ✅ No new store shape needed |
| **View hook** | Create `useViewTracking` with IntersectionObserver + 5s timer | ✅ Keeps components clean |
| **Accessibility** | `title` alone insufficient — MUST use `aria-label` | ✅ Add `aria-label` to all icon buttons |
| **Performance** | `filter: drop-shadow()` expensive → use `box-shadow` instead | ✅ Pseudo-element with `box-shadow` |
| **Reduced motion** | Disable pulsing/animated glows | ✅ Check `prefers-reduced-motion` |
| **Decay** | Keep in `useSentimentState` — already implemented (lines 52-54) | ✅ Reuse existing decay formula |
| **Scope** | Achievable (~1 day) | ✅ Proceed |
...
```
apps/web-pwa/
├── tailwind.config.cjs                    # Light mode tokens (optional)
├── src/hooks/
│   ├── useViewTracking.ts                 # NEW: 5s + scroll → recordRead
│   └── useSentimentState.ts               # Reuse existing recordRead/decay
├── src/components/
│   ├── EngagementIcons.tsx                # NEW: outline/solid + box-shadow glow
│   ├── AnalysisView.tsx                   # Icons top-right
│   └── hermes/
│       ├── forum/ThreadView.tsx           # useViewTracking, icons top-right
│       ├── CommentCard.tsx                # Show icons when isExpanded
│       └── CommunityReactionSummary.tsx   # Nested card, aria-label icons
```
...
**Phase 6 (Complete):**
- [x] Design unification implemented ✅
- [x] Borderless styling applied ✅
- [x] 3D flip with CSS Grid ✅
- [x] Surface tokens ✅
- [x] Tests passing (291), 100% coverage ✅

**Phase 6.1 (Complete — Manual Testing Refinements):**
- [x] Conversation summary → nested card, lighter font ✅
- [x] Engagement icons → glow system (outline/solid) ✅
- [x] Action icons → tooltip only, BeakerIcon ✅
- [x] View tracking (5s + scroll) ✅
- [x] All Heroicons (EyeIcon, LightBulbIcon, BeakerIcon, etc.) ✅

**Phase 6.2 (Complete — Dev Color Panel):**
- [x] Created comprehensive DevColorPanel component ✅
- [x] **57 CSS variables** for complete color control ✅
- [x] Separate page backgrounds for VENN/HERMES/AGORA ✅
- [x] **Section containers** (Headlines/Forum wrapper) controllable ✅
- [x] All components wired to CSS variables ✅
- [x] **Saturation, Lightness, Opacity sliders** per color ✅
- [x] Thread list cards (forum feed) fully controllable ✅
- [x] Icon shadow behind glow for edge definition ✅
- [x] Export CSS to clipboard feature ✅
- [x] **💾 Save as Default** — copies Tailwind config to clipboard ✅
- [x] localStorage persistence ✅
- [x] Category tabs for organization ✅
- [x] 296 tests, 100% coverage ✅

**Phase 6.3 (Complete — ThreadView Consolidation, Dec 11, 2025):**
- [x] 3D flip card visibility fix — hidden face no longer bleeds through transparent backgrounds ✅
- [x] Forum engagement wiring — `recordEngagement(topicId)` method added to useSentimentState ✅
- [x] Forum votes/comments now trigger lightbulb engagement icon ✅
- [x] Comment form auto-closes after posting ✅
- [x] Comments appear in real-time (fixed array mutation bug in `addComment`) ✅
- [x] Removed "Refresh Summary" button — auto-generation at thresholds (10, 20, 40, 60...) ✅
- [x] Unified ThreadView to single card — thread header + summary + debate columns ✅
- [x] Action icons (Send/Proposal/Project) moved inside summary card ✅
- [x] Debate columns (Concur/Counter) moved inside summary card ✅
- [x] Smooth zoom-in animation on ThreadView entry (respects `prefers-reduced-motion`) ✅
- [x] 298 tests, 100% line/branch/function/statement coverage ✅

**Phase 7.0 (Complete — Tri-State Stance Schema, Dec 12, 2025):**
- [x] Schema updated: `stance: 'concur' | 'counter' | 'discuss'` ✅
- [x] "Discuss" stance for neutral/questioning comments ✅
- [x] AI summary prompt updated to include discuss comments ✅
- [x] Concur/Counter percentages exclude discuss (debate clarity) ✅

**Phase 7.1 (Complete — Threaded CommentStream + SlideToPost, Dec 12-18, 2025):**
- [x] Replaced bifurcated Concur/Counter columns with single `CommentStream` ✅
- [x] Stance is metadata (badge + border color), not layout structure ✅
...
- [x] Card border radius matches connector side (rounded-r/l/full) ✅
- [x] Progressive width reduction: 92% → 90% → 88% → 85% (capped at depth 3+) ✅
- [x] Universal collapse/expand for all comment branches ✅
- [x] Depth ≥3 auto-collapses by default (with userToggled guard) ✅
- [x] **Tree Connector Refinements (Dec 18):**
  - **YouTube-style trunk termination:** Trunk ends at last child on each side (no overshoot)
  - **Double-curve for discuss:** When discuss comment's desired trunk offset differs from parent, draws step-in curve (outer → inner → card)
...
- [x] **SlideToPost** component — single gesture submission:
  - Gradient track (Support ← Discuss → Oppose)
  ...
  - Success feedback: "✓ Posted!" with 1.2s reset
- [x] Reply button wrapped in TrustGate ✅
- [x] Deleted `DebateColumn.tsx` (replaced by CommentStream) ✅
...
- [x] **LOC Refactor (Dec 18):**
  - Extracted `treeConnectors.tsx` (90 lines) — constants, helpers, BranchConnector
  - Extracted `VoteControl.tsx` (41 lines) — voting UI component
  - Extracted `commentStreamLayout.tsx` (25 lines) — cardMaxWidth, stanceMeta, ChildrenContainer
  - Added `treeConnectors.test.tsx` (6 tests) + `VoteControl.test.tsx` (5 tests)
  - `CommentStream.tsx` now 346 lines (under 350 cap)
- [x] 335 tests, 100% line/branch/function/statement coverage ✅
