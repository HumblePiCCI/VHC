    ...
    <div className="grid grid-cols-2 gap-2 rounded border border-slate-600/50 bg-slate-900/40 p-3">
      ...
      <div className="col-span-2 flex items-center justify-between gap-2 text-xs text-slate-300">
        <div className="flex items-center gap-1">
          <ToggleButton label="Disagree" active={agreement === -1} onClick={() => handleSet(-1)} ariaLabel="Disagree">
            –
          </ToggleButton>
          <ToggleButton label="Neutral" active={agreement === 0} onClick={() => handleSet(0)} ariaLabel="Neutral">
            ○
          </ToggleButton>
          <ToggleButton label="Agree" active={agreement === 1} onClick={() => handleSet(1)} ariaLabel="Agree">
            +
          </ToggleButton>
        </div>
...
function ToggleButton({
  ...
  return (
    <button
      type="button"
      className={`rounded px-2 py-1 ${active ? 'bg-teal-600 text-white' : 'bg-slate-800 text-slate-200 hover:bg-slate-700'}`}
      onClick={onClick}
